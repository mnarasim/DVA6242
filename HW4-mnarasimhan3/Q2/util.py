...
def entropy(class_y):
    ...
    count_0 = (class_y == 0).sum()
    count_1 = (class_y == 1).sum()
   
    if count_0 == len(class_y) or count_1 == len(class_y):
        ent = 0
    else:
        
        ent = - count_0/len(class_y) * math.log(count_0/len(class_y),2) - count_1/len(class_y) * math.log(count_1/len(class_y),2)
    return ent
...
def information_gain(previous_y, current_y):
    ...
    x1_ent = entropy(current_y[0])

    x2_ent = entropy(current_y[1])

    info_gain = prev_ent - (x1_ent*len(current_y[0])/len(previous_y) + x2_ent * len(current_y[1])/len(previous_y))
