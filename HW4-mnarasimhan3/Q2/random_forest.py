...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        samples = XX[ind,:-1] # sampled dataset
        labels = XX[ind,-1]  # class labels for the sampled records
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                votes2 =[]
                for i in range(len(self.bootstraps_datasets)):
                    votes2.append(self.decision_trees[i].classify(record))
                counts2 = np.bincount(votes2)
                y = np.append(y, np.argmax(counts2))
                pass
            else:
